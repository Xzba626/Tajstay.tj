...
import { createModeratorOfflineBooking } from "@/lib/moderator/offlineBooking";
...
export async function POST(req: NextRequest) {
  ...
  const room = roomId ? await prisma.room.findFirst({ where: { id: roomId }, select: { id: true, roomTypeId: true } }) : null;
  ...
  try {
    const booking = await createModeratorOfflineBooking(user.id, {
      roomTypeId,
      roomId: room?.id ?? null,
      checkIn,
      checkOut,
