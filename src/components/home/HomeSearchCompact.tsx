...
import { Calendar, ChevronRight, MapPin, Minus, Plus, Search, Users, X } from "lucide-react";
...
export function HomeSearchCompact({ locale }: Props) {
  ...
  return (
    <>
      <div className="home-search-compact md:hidden">
        <button type="button" className="home-search-compact__row" onClick={() => { haptic(); setSheet("city"); }}>
          <MapPin size={18} className="shrink-0 text-[var(--green-accent)]" aria-hidden />
          <div className="min-w-0 flex-1">
            <div className="home-search-compact__row-label">{m(locale, "search.placeholder")}</div>
        ...
        <button type="button" className="home-search-compact__row" onClick={() => { haptic(); setSheet("dates"); }}>
          <Calendar size={18} className="shrink-0 text-[var(--green-accent)]" aria-hidden />
          <div className="min-w-0 flex-1">
            <div className="home-search-compact__row-label">{m(locale, "search.dates")}</div>
            <div className="home-search-compact__row-value truncate">{datesLabel}</div>
          </div>
          <ChevronRight size={18} className="shrink-0 text-[var(--text-muted)]" aria-hidden />
        </button>

        <button type="button" className="home-search-compact__row" onClick={() => { haptic(); setSheet("guests"); }}>
          <Users size={18} className="shrink-0 text-[var(--green-accent)]" aria-hidden />
          <div className="min-w-0 flex-1">
            <div className="home-search-compact__row-label">{m(locale, "search.guests")}</div>
            <div className="home-search-compact__row-value">{guestsLabel}</div>
          </div>
          <ChevronRight size={18} className="shrink-0 text-[var(--text-muted)]" aria-hidden />
        </button>

        <div className="home-search-compact__submit-wrap">
          <button type="button" className="btn-primary" onClick={submitMobile}>
          ...
          <div className="bottom-sheet md:hidden" role="dialog" aria-modal="true">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-[var(--text-primary)]">
                {sheet === "city"
                  ? m(locale, "search.placeholder")
                  : sheet === "dates"
                    ? m(locale, "search.dates")
                    : m(locale, "search.guests")}
              </h3>
              <button type="button" onClick={closeSheet} className="rounded-lg p-2 text-[var(--text-muted)]" aria-label={m(locale, "common.close")}>
                <X size={20} />
              </button>
            </div>
            ...
            {sheet === "dates" ? (
              <div className="space-y-4">
                <label className="block">
                  <span className="premium-label">{m(locale, "search.checkIn")}</span>
                  <input type="date" className="premium-input" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} />
                </label>
                <label className="block">
                  <span className="premium-label">{m(locale, "search.checkOut")}</span>
                  <input type="date" className="premium-input" value={checkOut} min={checkIn || undefined} onChange={(e) => setCheckOut(e.target.value)} />
                </label>
                <button type="button" className="btn-primary" onClick={() => { haptic(); closeSheet(); }}>
            ...
            {sheet === "guests" ? (
              <div className="flex items-center justify-between gap-4 py-2">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--border)] text-[var(--green-accent)]" onClick={() => { haptic(); setGuests((g) => Math.max(1, g - 1)); }} aria-label="-">
                    <Minus size={18} />
                  </button>
                  <span className="min-w-[2rem] text-center text-lg font-bold text-[var(--text-primary)]">{guests}</span>
                  <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--border)] text-[var(--green-accent)]" onClick={() => { haptic(); setGuests((g) => Math.min(12, g + 1)); }} aria-label="+">
                    <Plus size={18} />
                  </button>
                </div>
              </div>
