    ...
    <div className="owner-upload-card">
      ...
      {preview ? (
        <div className="mt-3 overflow-hidden rounded-xl border border-white/12 bg-black/20">
          ...
          <img src={preview} alt="" className="h-32 w-full object-cover sm:h-36" />
          <div className="flex items-center justify-between gap-2 border-t border-white/10 px-3 py-2">
            <span className="truncate text-xs text-slate-300">{fileName}</span>
            <button type="button" onClick={clear} className="text-xs font-semibold text-red-300 hover:text-red-200">
              {removeLabel}
            </button>
          </div>
        </div>
