    ...
    <>
      ...
      <div className="relative rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.95),rgba(6,12,22,0.98))] p-5 text-white shadow-[0_24px_60px_rgba(0,0,0,0.45)] sm:p-6">
        ...
        <div className="mb-5 text-center">
          <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-[20px] bg-[linear-gradient(135deg,#0f7a4d,#166534)] shadow-lg shadow-[#0f3d2e]/40 ring-1 ring-white/15">
          ...
          <p className="mt-1.5 text-sm text-[#d1fae5]/75">Скопируй номер и открой приложение</p>
          {amountTjs != null && Number.isFinite(amountTjs) && (
            <p className="mt-3 text-lg font-bold text-white">
              К оплате:{" "}
              <span className="tabular-nums text-[#d1fae5]">
        ...
        <div
          className={`relative mb-4 overflow-hidden rounded-2xl border p-3.5 transition-colors sm:p-4 ${
            copied ? "border-[#0f7a4d]/50 bg-[#0f7a4d]/15" : "border-white/10 bg-black/20"
          }`}
        >
          <div className="flex items-center gap-2">
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-medium uppercase tracking-wide text-[#d1fae5]/60">Номер карты</p>
              <p className="mt-1 break-all font-mono text-base font-bold tracking-tight text-white sm:text-lg">{compact}</p>
              <p className="mt-1 text-xs text-[#d1fae5]/90">Мухаммадали Р. А.</p>
            </div>
            <button
              type="button"
              onClick={copyNumber}
              className={`shrink-0 rounded-xl px-3 py-2 text-xs font-bold transition active:scale-95 sm:text-sm ${
                copied ? "bg-[#0f7a4d] text-white" : "bg-[#d1fae5] text-slate-900 hover:bg-white"
              }`}
            >
              {copied ? "✓ OK" : "Copy"}
            </button>
        ...
        {copied && (
          <div className="mb-4 rounded-2xl border border-[#0f7a4d]/20 bg-[#0f7a4d]/10 px-3 py-3 text-center text-sm">
            <div className="font-semibold text-[#d1fae5]">Номер скопирован</div>
            <div className="mt-0.5 text-[#d1fae5]/80">Открой DC Next и заверши перевод</div>
          </div>
        )}
        ...
        <button
          ...
          className={`w-full rounded-2xl px-4 py-3.5 text-base font-bold transition active:scale-[0.99] ${
            copied
              ? "bg-[linear-gradient(135deg,#0f7a4d,#0f2920)] text-white shadow-lg shadow-[#0f2920]/40"
        ...
        {opened && <div className="mt-3 text-center text-2xl text-[#0f7a4d]/90">↓</div>}
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] bg-[linear-gradient(180deg,#03110b_0%,#0a2418_42%,#0c1f17_100%)] px-3 py-5 sm:px-4">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-20 top-0 h-56 w-56 rounded-full bg-[#0f7a4d]/15 blur-3xl" />
